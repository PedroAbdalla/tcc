...
function abrirCategoria(c) {
    ...
    repositorio = jQuery(c).attr('rsp');
    $.ajax({
        url: `../../tcc/listar_imagens_prancha/${valorCategoria}/${repositorio}`,
        type: 'get',
        dataType: 'html',
    })
    .done(function(retorno) {
...
function listarImagensPadrao(e) {
    let id_categoria = jQuery(e).find('option:checked').val();
    let repositorio = jQuery(e).find('option:checked').attr('data-repository');
    jQuery('.id-categoria-imagem').val(id_categoria)
    $.ajax({
        url: '../../tcc/listar_imagens_padrao/'+id_categoria,
        type: 'POST',
        dataType: 'HTML',
    })
    .done(function(retorno) {
...
function apagarImagem(id, cat) {
    if (cat == 1) {
        url = 'apagar_imagem_padrao';
    } else {
        url = 'apagar_imagem';
    }
    $.ajax({
